...
def optimized_triple_ensemble_trading(
    ...
    def optimize_thresholds(buy_threshold, sell_threshold):
        params = {
            'buy_threshold': np.clip(buy_threshold, 0.4, 0.5),
            'sell_threshold': np.clip(sell_threshold, 0.3, 0.45)
        }

        portfolio = backtest_strategy(val_df, val_scaled, params)
        metrics = calculate_metrics(portfolio)
        return metrics['Prediction Accuracy']

    optimizer = BayesianOptimization(
        f=optimize_thresholds,
        pbounds={
            'buy_threshold': (0.4, 0.5),
            'sell_threshold': (0.3, 0.45)
        },
        random_state=random_seed
    )
    ...
    def predict_tomorrow(latest_df):
        ...
        final_prob = 0.2*lstm_pred + 0.1*gru_pred + 0.7*rf_pred
